...
if __name__ == '__main__':
    ...
    def calc_score(i: int, j: int) -> float:
    ...
    for im_idx in range(num_images):
        ...
        view_sel.append([(k, score[im_idx, k]) for k in sorted_score[:args.num_src_images]])
    print('view_sel[0]\n', view_sel[0], end='\n\n')
    ...
    with open(os.path.join(args.output_folder, 'pair.txt'), 'w') as f:
        f.write('%d\n' % len(images))
        for im_idx, view_scores in enumerate(view_sel):
            f.write('%d\n%d ' % (im_idx, len(view_scores)))
            for image_id, view_score in view_scores:
                f.write('%d %f ' % (image_id, view_score))
            f.write('\n')
    for im_idx in range(num_images):
