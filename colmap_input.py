...
def read_cameras_binary(path_to_model_file):
    ...
    with open(path_to_model_file, 'rb') as fid:
        ...
        for camera_line_index in range(num_cameras):
            ...
            model_cameras[cam_id] = Camera(id=cam_id,
                                           model=model_name,
                                           width=width,
                                           height=height,
        ...
        while True:
            ...
            if len(line) > 0 and line[0] != '#':
                ...
                model_images.append(Image(
                    ...
                    xys=xys, point3D_ids=point_3d_ids))
    return model_images
    ...
    with open(path_to_model_file, 'rb') as fid:
        ...
        for image_index in range(num_reg_images):
            ...
            while current_char != b'\x00':  # look for the ASCII 0 entry
                ...
                current_char = read_next_bytes(fid, 1, 'c')[0]
            num_points_2d = read_next_bytes(fid, num_bytes=8,
                                            format_char_sequence='Q')[0]
            x_y_id_s = read_next_bytes(fid, num_bytes=24 * num_points_2d,
...
def read_points3d_binary(path_to_model_file):
    ...
    with open(path_to_model_file, 'rb') as fid:
        ...
        for point_line_index in range(num_points):
            ...
            point_3d_id = binary_point_line_properties[0]
...
if __name__ == '__main__':
    ...
    for im_idx in range(num_images):
        ...
        for p3d_id in images[im_idx].point3D_ids:
            if p3d_id == -1:
                continue
            transformed = np.matmul(extrinsic[im_idx],
    ...
    for idx1, idx2 in queue:
        s = calc_score(idx1, idx2)
        score[idx1, idx2] = s
        score[idx2, idx1] = s
    view_sel = []
    ...
    for im_idx in range(num_images):
        with open(os.path.join(cam_dir, '%08d_cam.txt' % im_idx), 'w') as f:
            ...
            for idx2 in range(4):
                for k in range(4):
                    f.write(str(extrinsic[im_idx][idx2, k]) + ' ')
                f.write('\n')
            f.write('\nintrinsic\n')
            for idx2 in range(3):
                for k in range(3):
                    f.write(str(intrinsic[images[im_idx].camera_id][idx2, k]) + ' ')
                f.write('\n')
            f.write('\n%f %f \n' % (depth_ranges[im_idx][0], depth_ranges[im_idx][1]))
    with open(os.path.join(args.output_folder, 'pair.txt'), 'w') as f:
        f.write('%d\n' % len(images))
        for im_idx, sorted_score in enumerate(view_sel):
            f.write('%d\n%d ' % (im_idx, len(sorted_score)))
            for image_id, s in sorted_score:
                f.write('%d %f ' % (image_id, s))
            f.write('\n')
    for im_idx in range(num_images):
        if args.convert_format:
            img = cv2.imread(os.path.join(image_dir, images[im_idx].name))
            cv2.imwrite(os.path.join(renamed_dir, '%08d.jpg' % im_idx), img)
        else:
            shutil.copyfile(os.path.join(image_dir, images[im_idx].name),
                            os.path.join(renamed_dir, '%08d.jpg' % im_idx))
